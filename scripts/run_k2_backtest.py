...
자동으로 다운로드한다). K2는 원 자료에 익절/손절 규칙이 없어 인접 기법(K1/K+)에서 차용한
신규 설계(stop_loss_pct=-7%, 전고점 익절, 4일차 강제청산)를 쓴다 — K2Config 참고.

사용 예:
    python scripts/run_k2_backtest.py --start 2021-01-01 --end 2024-12-31 --min-market-cap 0 --cache-dir .cache/ohlcv
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="K2 전략 백테스트")
    ...
    print(f"[run_k2_backtest] 유니버스 {len(tickers)}종목 로딩 중...")
    ohlcv = load_universe_ohlcv(tickers, args.start, args.end, cache_dir=args.cache_dir)
    print(f"[run_k2_backtest] {len(ohlcv)}종목 데이터 확보 완료. 백테스트 시작...")
    ...
    result = run_k2_backtest(ohlcv, config)
    m = result.metrics
    s = m.signal_level
