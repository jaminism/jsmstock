...
자동으로 다운로드한다). S2은 원 자료에 명시적 가격 손절가가 없어 S3와 동일하게 인접 기법에서
차용한 신규 설계(stop_loss_pct=-7%, 전고점 익절, 4일차 강제청산)를 쓴다 — S2Config 참고.
S3와의 핵심 차이는 진입 방식("종가베팅", D+1~D+2 한정)이다.

사용 예:
    python scripts/run_s2_backtest.py --start 2021-01-01 --end 2024-12-31 --min-market-cap 0 --cache-dir .cache/ohlcv
"""
...
from rich_stock.backtest.engine import run_s2_backtest
...
from rich_stock.strategies.s2 import detect_s2_signals


def main() -> None:
    parser = argparse.ArgumentParser(description="S2 전략 백테스트")
    ...
    print(f"[run_s2_backtest] 유니버스 {len(tickers)}종목 로딩 중...")
    ohlcv = load_universe_ohlcv(tickers, args.start, args.end, cache_dir=args.cache_dir)
    print(f"[run_s2_backtest] {len(ohlcv)}종목 데이터 확보 완료. 백테스트 시작...")
    ...
    result = run_s2_backtest(ohlcv, config)
    m = result.metrics
    s = m.signal_level
    ...
    if args.db:
        run_id = db.save_backtest_results(
            args.db, "S2", args.start, args.end, config, ohlcv, result.trades,
            detect_s2_signals, trades_csv_path=args.trades_csv,
        )
        print(f"DB 저장 완료: run_id={run_id}  db={args.db}")

    print(
        "\n⚠️ 본 결과는 S2 원 자료에 없는 명시적 손절가(인접 기법에서 차용한 신규 설계)를 포함하며, "
