...
from rich_stock.strategies.k1 import backtest_ticker, detect_k1_signals
...
def test_detect_signal_computes_fixed_k1_grid():
    ...
    # K1 = 13000 - (13000-9800)*0.236 = 12244.8
    ...
    signals = detect_k1_signals(df, K1Config())
    assert len(signals) == 1
    sig = signals[0]
    assert sig.ul_index == 1
    assert sig.high == 13000
    assert sig.low == 9800
    assert round(sig.k1_level) == round(13000 - (13000 - 9800) * 0.236)


def test_entry_at_close_when_close_breaches_k1_within_window():
    # K1 = 13000 - (13000-9800)*0.236 = 12244.8
    # day2(D+1) 종가 12500 > K1선 -> 미체결. day3(D+2) 종가 11800 <= K1선 -> 종가(11800)에 체결
    ...
    df = make_df(closes, highs=highs, lows=lows)
    trades = backtest_ticker(df, "TEST", K1Config())
    assert len(trades) == 1
    trade = trades[0]
    assert trade.fills[0].reason == "entry_k1"
...
def test_no_entry_after_d2_window_even_if_close_breaches_later():
    # D+1, D+2(index2,3) 종가 모두 K1선(12244.8) 위 -> 진입 없음. D+3(index4)에 하회해도 무효.
...
def test_stop_loss_minus_7_percent_from_entry_close():
    # entry: day2(D+1) 종가 12000 <= K1(12244.8) -> 12000에 체결
...
def test_forced_exit_after_hold_days():
    ...
    df = make_df(closes, highs=highs, lows=lows)
    trades = backtest_ticker(df, "TEST", K1Config(hold_days=4))
    assert len(trades) == 1
    trade = trades[0]
...
def test_pre_rally_lookback_widens_low_anchor():
    ...
    # 그 결과 K1 레벨도 낮아져야 한다(진입이 더 어려워짐).
    closes = [9500, 9200, 10000, 13000]
    highs = [9500, 9200, 10000, 13000]
    lows = [9000, 9100, 9800, 12000]
    df = make_df(closes, highs=highs, lows=lows)

    narrow = detect_k1_signals(df, K1Config(pre_rally_lookback_days=1))[0]
    wide = detect_k1_signals(df, K1Config(pre_rally_lookback_days=5))[0]

    assert narrow.low == 12000  # UL 당일 저가만 봄
    assert wide.low == 9000  # day0의 저가까지 포함
    assert wide.k1_level < narrow.k1_level
