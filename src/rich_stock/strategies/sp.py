...
규칙 출처: research/step_4_SP기법.md §2(원문 8개 규칙 + Q&A).
SR/K1/K2/K1+/K2+와 구조적으로 가장 다른 기법이다:

  1. **진입 트리거가 단일 상한가가 아니라 "3연속상한가 이상 + 고점대비 200~300% 상승" 랠리
     이벤트다.** `detect_sp_signals`가 연속 상한가 스트릭을 찾고, 스트릭 종료 후 `peak_search_days`
     이내 최고가를 "고점"(RH 역할)으로, 스트릭 시작 전 `pre_rally_lookback_days` 구간 최저가를
     "랠리 시작 저점"으로 삼아 누적상승률을 계산한다. 두 값 모두 고점 발생 시점에 고정되며,
     이후 갱신되지 않는다(SR/K1/K2의 R0~R3/K1선/K2선 고정 방식과 동일한 원칙).
  2. **되돌림 기준선이 피보나치 비율이 아니라 실제 이동평균선(15일/20일)이다.** 이동평균은
     ...
     체결되는 것으로 근사 — K2의 "장중 터치" 관례를 그대로 따름).
  3. **손절이 원문에 아예 없다 — 공백이 아니라 설계 의도.** SR/K1/K2/K1+/K2+는 전부 원문에
  ...
  4. **시간청산 기준점이 "2차(20일선) 매수 시점"이며, 1차(15일선)만 보유 중이면 시간청산도
     ...
     경로가 열려 있다(원문 Q&A에 명시된 내용을 그대로 반영 — 버그 아님, `SPConfig.hold_days`
...
def detect_sp_signals(df: pd.DataFrame, config: SPConfig) -> list[SPSignal]:
...
def simulate_sp_trade(
    ...
    """단일 SP 신호에 대해 진입 이후 흐름을 일봉 기준으로 시뮬레이션한다."""
    n = len(df)
    ...
    if shares > 1e-9:
        ...
        # 그대로 적용한다(버그 아님, SPConfig.hold_days docstring 참고).
        trade.fills.append(Fill(df.index[-1], df["Close"].iloc[-1], -shares, "exit_data_end"))

    return trade


def backtest_ticker(df: pd.DataFrame, ticker: str, config: SPConfig | None = None) -> list[Trade]:
    """단일 종목 전체 기간에 대해 SP 트레이드 목록을 생성한다."""
    config = config or SPConfig()
    ...
    signals = detect_sp_signals(df, config)
    trades = []
    for sig in signals:
        trade = simulate_sp_trade(df, sig, config, ticker, ma_short, ma_long)
