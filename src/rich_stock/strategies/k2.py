"""K2 전략 — 신호 탐지 및 개별 종목 트레이드 시뮬레이션.

규칙 출처: research/step_2_K2기법.md §2, §3.
이번 구현이 원문 규칙에 추가/보완/단순화한 부분:

  1. **익절/손절 규칙은 원문에 없다.** K2Config 참고 — step_3(K1)/step_5(K+) 문서의 인접 패턴
     ("-7% 손절", "전고점까지 익절", "4일차 강제청산")을 차용해 신규 설계했다. K2 원문에서
     검증된 값이 아니라는 점을 결과 해석 시 반드시 감안해야 한다.
  2. **되돌림 기준선(K2)을 SR의 R0~R3 그리드와 동일하게 상한가 발생일에 고정한다.** 원문은
  ...
  3. **HD<4(신고가 갱신 시 되돌림선 재산정) 로직은 생략**했다. 원문은 상한가 이후 신고가가
     경신되면 그 신고가를 새 고점으로 되돌림선을 다시 긋지만, 이번 버전은 SR과의 구조적 일관성과
     단순성을 위해 상한가 당일 고가를 고정 고점으로 사용한다.
  4. **K1(0.236, D<2 한정 유효) 신호는 이번 모듈에서 다루지 않는다** — step_3 리서치가 K1을
     "종가베팅"이라는 별도 진입 방식(장중 터치가 아닌 종가 매수, Stochastic 스크리닝 등)을 가진
     사실상 다른 기법으로 취급하고 있어, 여기서는 "K2 기법"(0.5 되돌림, 장중 터치 매수) 범위만
     구현한다.
  5. 익절은 "전고점까지 분할매도"를 SR과 동일하게 잔여 물량 전량 매도로 단순화했다.
  6. 손절은 "-7% 하락시 추매(대장주) 또는 손절"에서 1등주 판정이 불가능해(SR의 qualitative.py와
...
def detect_k2_signals(df: pd.DataFrame, config: K2Config) -> list[K2Signal]:
    """상한가(신규) 이벤트를 찾아 고정된 K2 되돌림선을 계산한다."""
    signals: list[K2Signal] = []
    is_ul = [
    ...
    for i in range(len(df)):
        ...
        k2_level = rh - (rh - rl) * config.fib_k2_ratio

        signals.append(K2Signal(ul_index=i, ul_date=df.index[i], high=rh, low=rl, k2_level=k2_level))
    return signals


def simulate_k2_trade(df: pd.DataFrame, signal: K2Signal, config: K2Config, ticker: str) -> Trade | None:
    """단일 K2 신호에 대해 진입 이후 흐름을 일봉 기준으로 시뮬레이션한다."""
    n = len(df)
    ...
    for i in range(signal.ul_index + 1, entry_window_end + 1):
        row = df.iloc[i]
        if row["Low"] <= signal.k2_level:
            entry_idx = i
            trade.fills.append(Fill(df.index[i], signal.k2_level, 1.0, "entry_k2"))
            break
    ...
    stop_price = signal.k2_level * (1 + config.stop_loss_pct)
...
def backtest_ticker(df: pd.DataFrame, ticker: str, config: K2Config | None = None) -> list[Trade]:
    """단일 종목 전체 기간에 대해 K2 트레이드 목록을 생성한다."""
    config = config or K2Config()
    if df.empty or len(df) < 5:
        return []
    signals = detect_k2_signals(df, config)
    trades = []
    for sig in signals:
        trade = simulate_k2_trade(df, sig, config, ticker)
