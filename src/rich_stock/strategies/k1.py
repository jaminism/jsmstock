"""K1 전략 — 신호 탐지 및 개별 종목 트레이드 시뮬레이션.

규칙 출처: research/step_3_K1기법.md §2, §3.
K2([[project-k2-backtest-engine]])와 동일한 피보나치 셋업(상한가 발생일에 고정된 RH/RL)을 공유하되,
되돌림 비율과 진입 방식이 다르다:

  1. **진입 방식이 K2와 다르다 — "종가베팅"**. K2는 장중 저가가 되돌림선을 터치하면 그 레벨
     가격에 체결(장중 지정가 매수)로 근사했지만, K1은 원문상 "K1선을 훼손하는 시점에 종가 부근에서
     매수"하는 종가 기준 기법이다(§2). 이번 구현은 해당 일자 **종가가 K1선을 하회(훼손)하면
     그날 종가로 체결**하는 것으로 근사한다.
  2. **매수 가능 기간이 D+1~D+2로 K2(D+7)보다 훨씬 짧다** (원문 §2-4 "매수 가능 기간: 상한가
     당일 제외, D+1·D+2까지만"). `K1Config.entry_valid_days=2`로 반영.
  3. RH/RL 고정 방식은 K2와 완전히 동일 — 상한가 당일 고가를 RH, 상한가 발생일 포함 직전
     lookback일 최저가를 RL로 고정한다(자기순환적 결함 회피).
  4. **익절/손절/시간청산 규칙은 K2와 동일하게 차용**: -7% 손절(원문의 "1등주면 추매, 아니면
  ...
  6. 추매(물타기, -7% 시 1등주 추가매수)는 1등주 판정 불가 한계로 미구현 — K2와 동일.
"""
...
def detect_k1_signals(df: pd.DataFrame, config: K1Config) -> list[K1Signal]:
    """상한가(신규) 이벤트를 찾아 고정된 K1 되돌림선을 계산한다."""
    signals: list[K1Signal] = []
    is_ul = [
    ...
    for i in range(len(df)):
        ...
        k1_level = rh - (rh - rl) * config.fib_k1_ratio

        signals.append(K1Signal(ul_index=i, ul_date=df.index[i], high=rh, low=rl, k1_level=k1_level))
    return signals


def simulate_k1_trade(df: pd.DataFrame, signal: K1Signal, config: K1Config, ticker: str) -> Trade | None:
    """단일 K1 신호에 대해 진입 이후 흐름을 일봉 기준으로 시뮬레이션한다."""
    n = len(df)
    ...
    for i in range(signal.ul_index + 1, entry_window_end + 1):
        row = df.iloc[i]
        if row["Close"] <= signal.k1_level:
            entry_idx = i
            trade.fills.append(Fill(df.index[i], float(row["Close"]), 1.0, "entry_k1"))
            break
...
def backtest_ticker(df: pd.DataFrame, ticker: str, config: K1Config | None = None) -> list[Trade]:
    """단일 종목 전체 기간에 대해 K1 트레이드 목록을 생성한다."""
    config = config or K1Config()
    if df.empty or len(df) < 5:
        return []
    signals = detect_k1_signals(df, config)
    trades = []
    for sig in signals:
        trade = simulate_k1_trade(df, sig, config, ticker)
