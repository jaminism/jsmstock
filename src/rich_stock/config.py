...
@dataclass(frozen=True)
class SPConfig(PortfolioConfig):
    """SP2(이동평균선 눌림목) 백테스트 파라미터.

    값의 출처는 research/step_4_SP기법.md §2(원문 8개 규칙 + Q&A)이다. **중요**: 원문은 명시적
    가격 손절이 없다는 것 자체가 설계 의도다("존버" — §7.3 risk-manager 검토). SR/K1/K2/K1+/K2+
    ...
    K2Config의 HD<4(신고가 갱신 시 재산정) 생략과 유사한 단순화."""
    ...
    """고점 발생 이후 1차/2차 매수가 유효한 기간. 원문에 명시적 기한이 없어(K1의 D+1~D+2 같은
    구체적 기한이 SP2 원문에는 없음) 신규 설계한 값 — 200~300%대 급등 이후 눌림이 형성되기까지
    시간이 걸릴 수 있다는 점을 감안해 SR/K1/K2(7일 이내)보다 넉넉하게 잡았다."""
    ...
    구현은 분할매도를 "절반 매도"로 단순화했다(SR의 본절매도 단순화와 동일한 방식)."""
