...
여러 기법(SR/K1/K2/K1+/K2+)이 CLI 스크립트를 각각 따로 실행할 때마다 결과가 stdout/CSV로만
남아 기법 간 비교(같은 날 SR·K2 신호가 겹치는지 등)나 나중 fund-manager 포트폴리오 구성 단계에서
...
스키마:
  ...
  signals(run_id, technique, ticker, event_date, high, low, level, extra)
    - `level`은 기법별 "1차 진입 트리거 가격"의 공통 명칭이다(SR=R1, K1=K1선, K2=K2선).
      K1+/K2+는 되돌림 비율(fib_ratio)이 변형(K1+/K2+)마다 달라 detect 단계에서 아직 확정되지
      않으므로 NULL로 둔다. 기법 고유 필드(SR의 R0~R3, 정성적 점수 등)는 `extra`에 JSON으로 넣는다.
...
def _signal_row(technique: str, ticker: str, sig: Any) -> dict:
    ...
    # 경우가 있다(특히 SR의 r0~r3) — DuckDB Python 바인딩이 numpy 스칼라를 직접 받아들이지
    # 못해 float()로 명시 캐스팅한다.
    if technique == "SR":
    ...
    if technique == "K1":
        return {"ticker": ticker, "event_date": sig.ul_date, "high": float(sig.high), "low": float(sig.low), "level": float(sig.k1_level), "extra": None}
    if technique == "K2":
        return {"ticker": ticker, "event_date": sig.ul_date, "high": float(sig.high), "low": float(sig.low), "level": float(sig.k2_level), "extra": None}
