...
def connect(db_path: str | Path = DEFAULT_DB_PATH, read_only: bool = False) -> duckdb.DuckDBPyConnection:
    ...
    **주의(2026-08-21 정정)**: read_only=True도 만능이 아니다 — DuckDB는 파일 하나에 쓰기
    ...
    #
    # **컬럼 존재 여부를 먼저 확인하고 필요할 때만 ALTER 실행 + 즉시 CHECKPOINT(2026-09-03
    # 수정, 두 단계에 걸쳐 확인)**: 처음엔 `ALTER TABLE ... ADD COLUMN IF NOT EXISTS`를
    # connect()마다 무조건 실행했다 — 컬럼이 이미 있어 SQL 결과상 no-op이어도 DuckDB는 이
    # 구문을 WAL에 계속 새로 기록했고, 데몬이 정상 종료 없이(docker restart의 SIGTERM 등)
    # 죽으면 그 WAL 항목이 커밋되지 않은 채 남아 다음 기동 시 재생 중 내부 오류로 크래시했다
    # (실전 사고, 데몬이 아예 뜨지 못하는 상태까지 감). 컬럼 존재 여부를 먼저 확인하는 걸로
    # 1차 수정했는데도 같은 사고가 재발했다 — 원인은 ALTER 자체가 **한 번도 메인 DB 파일에
    # 체크포인트되지 않고 계속 WAL에만 남아있었던 것**이라, 매 기동마다 존재 확인이 "없음"으로
    # 나와 다시 ALTER를 실행하고 있었다. ALTER 직후 명시적으로 CHECKPOINT를 호출해 WAL을
    # 즉시 메인 파일에 병합·비워야, 다음 기동부터는 컬럼이 진짜로 존재해 이 분기 자체를 아예
    # 타지 않는다.
